...
function __construct()
	...
	public function contarPuntaje($frame1,$frame2,$frame3,$frame4,$frame5,$frame6,$frame7,$frame8,$frame9,$frame10){
		...
		for ($i=0; $i < 10; $i++) { 
		//print_r ($array[i][0]); 	
		if ((($array[$i][0][0]+$array[$i][1][1])==$aux) && (($array[$i+1][0][0]+$array[$i+1][1][1])==$aux)) {
			
		$sumaFrame=$sumaFrame+$array[$i][0]+$array[$i+1][0]+$array[$i+2][0]+$array[$i+2][1];
			
		}elseif ((($array[$i][0][0]+$array[$i][1][1])==$aux) && (($array[$i+1][0][0]+$array[$i+1][1][1])<$aux)) {
			$sumaFrame=$sumaFrame+$array[$i][0]+$array[$i+1][0]+$array[$i+1][1];
		}else{
			$sumaFrame=$sumaFrame+$array[$i+1][0]+$array[$i+1][1];
		}	
		}
